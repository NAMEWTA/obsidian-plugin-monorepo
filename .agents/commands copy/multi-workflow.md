...
# New session call
Bash({
  command: "~/.agents/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: "~/.agents/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Analysis | `~/.agents/.ccg/prompts/codex/analyzer.md` | `~/.agents/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.agents/.ccg/prompts/codex/architect.md` | `~/.agents/.ccg/prompts/gemini/architect.md` |
| Review | `~/.agents/.ccg/prompts/codex/reviewer.md` | `~/.agents/.ccg/prompts/gemini/reviewer.md` |
...
**Claude Synthesis**: Adopt Codex backend plan + Gemini frontend plan, save to `.agents/plan/task-name.md` after user approval.
