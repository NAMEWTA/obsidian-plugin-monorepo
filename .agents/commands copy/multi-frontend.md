...
# New session call
Bash({
  command: "~/.agents/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview - \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: "~/.agents/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Phase | Gemini |
|-------|--------|
| Analysis | `~/.agents/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.agents/.ccg/prompts/gemini/architect.md` |
| Review | `~/.agents/.ccg/prompts/gemini/reviewer.md` |
...
- ROLE_FILE: `~/.agents/.ccg/prompts/gemini/analyzer.md`
- Requirement: Enhanced requirement (or $ARGUMENTS if not enhanced)
...
- ROLE_FILE: `~/.agents/.ccg/prompts/gemini/architect.md`
...
**MUST call Gemini** (follow call specification above):
- ROLE_FILE: `~/.agents/.ccg/prompts/gemini/reviewer.md`
