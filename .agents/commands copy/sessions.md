...
Manage Claude Code session history - list, load, alias, and edit sessions stored in `~/.agents/sessions/`.
...
```bash
node -e "
const sm = require((process.env.agents_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.agents'))+'/scripts/lib/session-manager');
const aa = require((process.env.agents_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.agents'))+'/scripts/lib/session-aliases');
...
console.log('Path: ~/.agents/sessions/' + session.filename);
console.log('');
...
- Sessions are stored as markdown files in `~/.agents/sessions/`
