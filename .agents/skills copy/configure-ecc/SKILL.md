...
2. **Manual**: Copy only this skill to `~/.agents/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
Options:
  - "User-level (~/.agents/)" — "Applies to all your Claude Code projects"
  - "Project-level (.agents/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=~/.agents`
- Project-level: `TARGET=.agents` (relative to current project root)
- Both: `TARGET_USER=~/.agents`, `TARGET_PROJECT=.agents`
...
```bash
grep -rn "~/.agents/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.agents/` paths:
- If a skill references `~/.agents/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.agents/skills/` or `~/.agents/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.agents/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.agents/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .agents/skills/")
...
- For user-level: check `~/.agents/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.agents/skills/<skill-name>/SKILL.md` exists
...
- Some skills assume `~/.agents/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.agents/homunculus/` directory is always user-level — this is expected and not an error.
